...
class UPNPService {
  ...
  async soapRequest(action, args, localAddress) {
    const type = this.serviceType;
    const req = this.createRequest(action, args);

    const res = await new Promise((resolve, reject) => {
      const protocol = this.controlURL.substring(0, 5) === 'https' ? Https : Http;
      const r = protocol.request(this.controlURL, {
        method: 'POST',
        timeout: UPNP.RESPONSE_TIMEOUT,
        headers: {
  ...
  async addPortMapping(remote, src, local, dest, protocol, ttl) {
  ...
  async removePortMapping(remote, local, port, protocol) {
